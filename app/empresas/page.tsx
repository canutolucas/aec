...
export default async function EmpresasPage() {
  ...
  async function criarEmpresa(formData: FormData) {
    ...
    if (error) redirect(comQuery(rotas.empresas, { erro: error.message }));
    redirect(rotas.contas((data as { id: string }).id));
  }
          ...
          <ul className="divide-y divide-[--color-borda]">
            {companies.map((company) => (
              <li key={company.id} className="flex items-center justify-between px-4 py-3">
                ...
                <LinkButton href={rotas.painel(company.id)}>Abrir</LinkButton>
              </li>
            ))}
          </ul>
        </Card>
